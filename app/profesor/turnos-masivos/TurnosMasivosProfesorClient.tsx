...
import {
  crearTurnosMasivosProfesor,
  crearAlumnoYTurnosMasivosProfesor,
  type SlotGenerado,
...
function generarSlots(
  ...
  while (cur <= hasta) {
    ...
    for (const d of disponibilidad.filter(x => x.activo && x.dia_semana === dow)) {
      ...
      for (let start = wStart; start < wEnd; start += INTERVALO) {
        for (const dur of DURACIONES) {
          ...
          slots.push({ profesorId, fecha: dateStr, hora_inicio: minToTime(start), hora_fin: minToTime(start+dur), duracion_minutos: dur });
        }
      }
    }
...
function slotKey(s: SlotGenerado) { return `${s.fecha}|${s.hora_inicio}|${s.duracion_minutos}`; }
...
export default function TurnosMasivosProfesorClient({
  ...
  const colegioParaTurno   = modoAlumno === "nuevo" ? nuevoAlumno.colegio : (alumnoActual?.colegio ?? "");
  ...
  function crear() {
    startTrans(async () => {
      ...
      if (modoAlumno === "existente") {
        r = await crearTurnosMasivosProfesor(alumnoId, materia, anio, colegioParaTurno, selectedSlots);
      } else {
        r = await crearAlumnoYTurnosMasivosProfesor(nuevoAlumno, materia, anio, selectedSlots);
      }
      setResult(r);
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl p-5 space-y-5" style={{boxShadow:"0 2px 12px rgba(124,58,237,0.07)"}}>
        ...
        {modoAlumno === "existente" ? (
          <div>
            ...
            <select value={alumnoId} onChange={e => setAlumnoId(e.target.value)} className={inputCls}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="text" placeholder="Nombre" value={nuevoAlumno.nombre}
                  onChange={e => setNuevoAlumno(p => ({...p, nombre: e.target.value}))} className={inputCls}/>
              </div>
              <div>
                <label className="block text-xs font-bold text-[#9ca3af] mb-1">Apellido *</label>
                <input type="text" placeholder="Apellido" value={nuevoAlumno.apellido}
                  onChange={e => setNuevoAlumno(p => ({...p, apellido: e.target.value}))} className={inputCls}/>
              </div>
              <div>
                <label className="block text-xs font-bold text-[#9ca3af] mb-1">DNI</label>
                <input type="text" placeholder="Ej: 45123456" value={nuevoAlumno.dni}
                  onChange={e => setNuevoAlumno(p => ({...p, dni: e.target.value}))} className={inputCls}/>
              </div>
              <div>
                <label className="block text-xs font-bold text-[#9ca3af] mb-1">Edad</label>
                <input type="number" placeholder="Ej: 12" min={4} max={80} value={nuevoAlumno.edad}
                  onChange={e => setNuevoAlumno(p => ({...p, edad: e.target.value}))} className={inputCls}/>
              </div>
              <div>
                <label className="block text-xs font-bold text-[#9ca3af] mb-1">Nivel educativo</label>
                <select value={nuevoAlumno.nivel_educativo}
                  onChange={e => setNuevoAlumno(p => ({...p, nivel_educativo: e.target.value}))} className={inputCls}>
        ...
        {/* Semana, materia, año */}
        <div className="border-t border-[#f3f4f6] pt-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div>
            <label className="block text-xs font-extrabold text-[#374151] mb-1">Semana (lunes)</label>
            <input type="date" value={semana}
              onChange={e=>{setSemana(e.target.value); setGenerated(false);}} className={inputCls}/>
          </div>
          <div>
          ...
          <div>
            ...
            <input type="text" placeholder="Ej: 3° año" value={anio}
              onChange={e=>setAnio(e.target.value)} className={inputCls}/>
          </div>
        </div>

        <button type="button" onClick={generar}
          className="px-5 py-2.5 rounded-xl bg-[#7c3aed] text-white text-sm font-black hover:bg-[#6d28d9] transition-colors">
          Ver mis turnos disponibles →
        </button>
      </div>
      ...
      {result && (
        <div className={`rounded-2xl p-4 font-bold text-sm ${
          ...
          result.errores===0 ? "bg-[#d1fae5] text-[#059669]" : "bg-[#fef3c7] text-[#d97706]"
        }`}>
          {result.error
            ? `❌ Error: ${result.error}`
      ...
      {generated && slots.length === 0 && (
        <div className="bg-white rounded-2xl py-10 text-center text-[#9ca3af] font-bold">
          Sin turnos disponibles esa semana en tu horario configurado.
        </div>
      )}
          ...
          <div className="px-5 py-4 bg-[#f5f3ff] border-b border-[#e5e7eb]">
            <div className="flex items-center justify-between flex-wrap gap-3">
              ...
              <div className="flex items-center gap-2">
                <button type="button" onClick={() => setSelSlots(new Set(slots.map(slotKey)))}
                ...
                <button type="button" onClick={() => setSelSlots(new Set())}
                  className="px-3 py-1.5 rounded-lg border border-[#e5e7eb] text-xs font-extrabold text-[#6b7280] hover:bg-[#f3f4f6] transition-colors">
                  Ninguno
                </button>
                <button type="button" disabled={!puedeCrear()} onClick={crear}
              ...
              return (
                <div key={fecha} className="px-5 py-4">
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-xs font-extrabold text-[#374151]">{dayName} {dayLabel}</p>
                    {selCount > 0 && (
                  ...
                  <div className="flex flex-wrap gap-2">
                    {ss.map(s => {
                      const key = slotKey(s); const sel = selSlots.has(key);
                      return (
                        <button key={key} type="button" onClick={() => toggleSlot(key)}
                          className={`px-3 py-2 rounded-xl text-xs font-extrabold border-2 transition-all ${
                            sel ? "border-[#7c3aed] bg-[#ede9fe] text-[#7c3aed]"
