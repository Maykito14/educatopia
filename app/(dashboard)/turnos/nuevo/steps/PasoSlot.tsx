...
export default function PasoSlot({ data, onChange, errors }: Props) {
  ...
  // Precio estimado para el slot seleccionado
  ...
  return (
    <div>
      ...
      <p className="text-sm text-[#6b7280] font-semibold mb-5">
      ...
      {/* Monto estimado — aparece al seleccionar un turno */}
      {montoEstimado !== null && (
      ...
      {profesores.length === 0 ? (
        ...
        <div className="flex flex-col gap-5">
          {profesores.map((prof) => {
            const slots = getSlotsPorProfesor(prof.id);
            const profSeleccionado = data.profesorId === prof.id;

            // Agrupar por fecha → hora_inicio → slots
            const porFecha = slots.reduce<Record<string, Record<string, MockSlot[]>>>(
              (acc, s) => {
                ...
                <div className="divide-y divide-[#f3f4f6]">
                  {fechas.length === 0 && (
                    <p className="text-sm text-[#9ca3af] font-semibold text-center py-4 px-4">
                      Sin turnos disponibles en los próximos 14 días.
                    </p>
                  )}
                  ...
                  {fechas.map((fecha) => {
                    ...
                    const diaSeleccionado = Object.values(porFecha[fecha])
                      .flat()
                      .some((s) => s.id === data.slotId);
